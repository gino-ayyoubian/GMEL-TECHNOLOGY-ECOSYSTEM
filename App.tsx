...
const MainAppLayout: React.FC = () => {
    const { activeView, setActiveView, region, setRegion, lang, setLang, supportedLangs } = useContext(AppContext)!;
    const { t } = useI18n();
    ...
    return (
        <div className="flex h-screen bg-slate-900 text-slate-100" dir={lang === 'fa' || lang === 'ar' || lang === 'ku' ? 'rtl' : 'ltr'}>
            <aside className={`w-64 flex-shrink-0 bg-slate-800 p-4 border-slate-700 flex flex-col ${lang === 'fa' || lang === 'ar' || lang === 'ku' ? 'border-l' : 'border-r'}`}>
